...
class Sidebar extends Component {
  ...
  handleDocumentClick = e => {
    ...
    if (e.target && e.target.classList && (e.target.classList.contains("menu-button") || e.target.classList.contains("menu-button-mobile"))) {
      ...
      (e.target.parentElement.parentElement.classList.contains("menu-button") || e.target.parentElement.parentElement.classList.contains("menu-button-mobile"))
    ) {
      isMenuClick = true;
    }
  ...
  setSelectedLiActive = callback => {
    ...
    if (selectedlink != null) {
      ...
      if (selectedParentNoSubItem != null) {
        ...
        this.setState(
          {
            selectedParentMenu: this.state.menuItems.length > 0 ? this.state.menuItems[0].id : "",
          },
          callback,
        );
      }
  ...
  filteredList = items => {
    ...
    if (currentUser) {
      return items.filter(x => (x.roles && x.roles.includes(currentUser.role)) || !x.roles);
    }
    return items;
  };
                ...
                this.filteredList(menuItems).map(item => {
                  return (
                    <Nav
                      ...
                      className={classnames({
                        "d-block":
                          (this.state.selectedParentMenu === item.id && this.state.viewingParentMenu === "") || this.state.viewingParentMenu === item.id,
                      })}
                      data-parent={item.id}>
                      {item.subs &&
                        this.filteredList(item.subs).map((sub, index) => {
                          return (
                            <NavItem key={`${item.id}_${index}`} className={`${sub.subs && sub.subs.length > 0 ? "has-sub-item" : ""}`}>
                              {sub.newWindow ? (
                                ...
                                <>
                                  <NavLink
                                    className={`rotate-arrow-icon opacity-50 ${collapsedMenus.indexOf(`${item.id}_${index}`) === -1 ? "" : "collapsed"}`}
                                    to={sub.to}
                                    id={`${item.id}_${index}`}
                                          ...
                                          <NavItem key={`${item.id}_${index}_${thirdIndex}`}>
                                            {thirdSub.newWindow ? (
                                              <a href={thirdSub.to} rel="noopener noreferrer" target="_blank">
                                                <i className={thirdSub.icon} /> {thirdSub.label}
                                              </a>
                                            ) : (
