...
function GlideComponent(props) {
  ...
  const renderDots = () => {
    ...
    for (let i = 0; i < total; i++) {
      dots.push(<button type="button" className="glide__bullet slider-dot" key={i} data-glide-dir={`=${i}`} />);
    }
    return dots;
  };
      ...
      <div className="glide" ref={node => (carousel = node)}>
        ...
        {!props.settings.hideNav && (
          <div className="glide__arrows slider-nav" data-glide-el="controls">
            ...
            <button type="button" className="glide__arrow glide__arrow--right right-arrow btn btn-link" data-glide-dir=">">
