...
const TopNavigation = ({ className, disableNav, topNavClick }) => {
  ...
  const itemClick = (stepItem, push, steps, step) => {
    if (disableNav) {
      return;
    }
    topNavClick(stepItem, push, steps, step);
  };

  return (
    <WithWizard
      render={({ next, previous, step, steps, go, push }) => (
        <ul className={`nav nav-tabs ${className}${disableNav ? " disabled" : ""}`}>
          {steps.map((stepItem, index) => {
            if (!stepItem.hideTopNav) {
              return (
                <li key={`topNavStep_${index}`} className={`nav-item ${getClassName(steps, step, index, stepItem)}`}>
                  <NavLink to="#" location={{}} className="nav-link" onClick={() => itemClick(stepItem, push, steps, step)}>
